...
def understand_query_node(state: SearchState) -> SearchState:
    ...
    understand_prompt = """你是一名智能搜索助手。请分析用户的查询，完成两个任务：
1. 简洁总结用户想要了解什么
...
搜索词：[最佳搜索关键词]"""
    ...
    response = llm.invoke([
        ...
        HumanMessage(content=f'请分析这个查询："{user_message}"')
    ])
...
def generate_answer_node(state: SearchState) -> SearchState:
    ...
    if state["step"] == "search_failed":
        # 如果搜索失败，基于 LLM 知识回答
        fallback_prompt = """搜索 API 暂时不可用，请基于您的知识回答用户的问题。
...
请提供一个有用的回答，并说明这是基于已有知识的回答。"""
    ...
    # 基于搜索结果生成答案
    answer_prompt = """你是一名专业的搜索助手。请基于搜索结果为用户提供完整、准确的答案。
